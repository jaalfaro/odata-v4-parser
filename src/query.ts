...
export function expand(value: SourceArray, index: number, metadataContext?: any): Lexer.Token {
  const start = index;
...
export function searchParenExpr(value: SourceArray, index: number): Lexer.Token {
  const open = Lexer.OPEN(value, index);
  if (!open) {
    return;
  }
...
export function allOperationsInSchema(value: SourceArray, index: number): number {
...
export function qualifiedActionName(value: SourceArray, index: number): Lexer.Token {
  const namespaceNext = NameOrIdentifier.namespace(value, index);
...
export function qualifiedFunctionName(value: SourceArray, index: number): Lexer.Token {
  const namespaceNext = NameOrIdentifier.namespace(value, index);
  if (namespaceNext === index || value[namespaceNext] !== 0x2e) {
    return;
  }
